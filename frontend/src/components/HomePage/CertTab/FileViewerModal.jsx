...
export const FileViewerModal = ({ show, onClose, certificate }) => {
  if (!show || !certificate) return null;
  const fileExtension = certificate.FileCC.split(".").pop().toLowerCase();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-lg max-w-5xl w-full h-[90vh] p-4 overflow-hidden flex flex-col">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">{certificate.TenCC}</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-hidden">
          <div className="flex-1 overflow-auto">
            {isImage ? (
              <img
                src={`${process.env.REACT_APP_BACKEND_URL}/${certificate.FileCC}`}
                alt={certificate.TenCC}
