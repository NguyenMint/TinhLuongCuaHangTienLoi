...
import { ChevronLeftIcon, ChevronRightIcon, SearchIcon, FileIcon, ChevronDownIcon } from 'lucide-react';
...
export const WorkSchedule = () => {
  ...
  return <div className="container mx-auto px-4 py-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
          ...
          <div className="flex flex-1 max-w-xl space-x-4">
            <div className="relative flex-1">
              <input type="text" placeholder="Tìm kiếm nhân viên" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500" />
              <SearchIcon className="absolute left-3 top-2.5 h-5 w-5 text-gray-400" />
            </div>
            <div className="relative">
              <select value={viewType} onChange={e => setViewType(e.target.value)} className="appearance-none w-48 pl-4 pr-10 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500">
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              ...
              <button onClick={handleThisWeek} className="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-50">
                Tuần này
              </button>
      ...
      {/* Schedule Table */}
      <ScheduleTable currentDate={currentDate} onAddShift={handleAddShift} />
      {/* Add Shift Modal */}
      {isModalOpen && <AddShiftModal isOpen={isModalOpen} onClose={handleModalClose} employee={selectedEmployee} date={selectedDate} />}
    </div>;
};